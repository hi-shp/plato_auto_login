...
* **최신 버전 (v2026.09.04)**: 
    ...
    * **PLATO 로그인 시 홈 화면 대신 '교과과정 페이지'로 자동 이동 지원**.
    * **교과과정 페이지 최상단 '스마트 학업 캘린더' 대시보드 위젯 탑재**:
        * 이번 달 전체 과제 및 동영상 강의(VOD) 마감일정 월간 캘린더 뷰 제공.
        * 과제 제출 완료 여부(`제출 완료`/`미제출`) 및 VOD 학습 완료 상태(`완료`/`미완료`) 실시간 추적 및 뱃지 표기.
        * 마감 임박 순 할 일 카드 리스트 및 직관적인 D-Day 뱃지 (`오늘 마감`, `D-1` 등), 바로가기 링크 제공.
        * '미완료만 보기' 원클릭 토글 필터 지원.
        * **5초 쿨다운 실시간 초 단위 카운트다운 새로고침 버튼** (자동 동기화 + 수동 새로고침).
